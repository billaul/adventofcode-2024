...
class Map
  ...
  VALUES = Hash.new {|h,v| h[v] = Float::INFINITY }
  DIRECTION = {
    '^' => [-1,0],
    '>' => [0,1],
    '<' => [0,-1],
  ...
  def solve(x=nil, y=nil, path = [])
    ...
    if self[x,y] == 'E'
      new_min_cost = [$min_cost, current_cost].min
...
puts
puts '======'
views =  $paths.flat_map { |path| path.map {|nb| {x: nb[:x], y: nb[:y]}} }.uniq

maze.display(views, char: 'O')
puts maze.to_s(views, char: 'O').chars.count {_1 == 'O'} + 1
# 631
